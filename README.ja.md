...
このパッケージは主に Linux 環境を想定していますが、`app_window_check_linux` 以外は Linux 依存はありません。動作確認済みの環境は Ubuntu 22.04 / 24.04 / 26.04 (リリース前) です。他の Linux ディストリビューションでも動作するはずです。
...
- テストは拡充中ですが網羅的ではありません — 重要なワークフローは手動で動作確認してください
